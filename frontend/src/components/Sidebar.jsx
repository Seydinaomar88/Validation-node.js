...
import { Menu, Home, Settings, Users, FileText, User2Icon, LogOut } from "lucide-react";
      ...
      <div className="drawer-content bg-gray-50 flex flex-col min-h-screen">
        <nav className="w-full bg-white border-b justify-between lg:justify-end border-gray-100 flex items-center py-3 px-8 shadow-sm">
          ...
          <div className="flex gap-3 items-center">
            <User2Icon size={30} className="text-gray-500" />
            <span className="text-base font-semibold text-gray-800">
              {user?.user?.name || user?.name || "Admin"}
            </span>
            <button
              onClick={handleLogout}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-500 bg-red-50 hover:bg-red-100 rounded-lg transition-colors"
            >
              <LogOut size={16} />
              Déconnexion
            </button>
